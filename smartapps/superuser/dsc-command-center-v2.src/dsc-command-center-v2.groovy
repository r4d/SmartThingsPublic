...
def lanResponseHandler(evt) {
    ...
    log.debug "response $evt.description"
    
    try {
        // Duration duration = Duration.between( lastHealthCheck , now );
        TimeDuration duration = TimeCategory.minus( stop, start )
        
        log.debug "healthcheck time check $duration.toString()"
...
def alarmStatusUpdate(evt) {
	...
	log.debug "update status with $evt.value"
...
private callAlarmServer(path) {
	try {
		log.debug "call alarm: $path, $ip, $port"
        
        //def result = new physicalgraph.device.HubAction("""POST /api/alarm/stayarm HTTP/1.1\r\nAccept: */* \r\nUser-Agent: Linux UPnP/1.0 SmartThings\r\nHOST: $ip:$port\r\nContent-Length:0\r\n\r\n """, physicalgraph.device.Protocol.LAN, "${deviceNetworkId}")
        
        httpPost(
        [
            uri: "http://$ip:$port$path?alarmcode=$alarmCodePanel.value"
        ]) { resp ->
